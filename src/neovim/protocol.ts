...
export interface Events {
  ...
  hl_attr_define(id: number, rgb_attrs: object, cterm_attrs: object, info: any[]): void,
  ...
  win_pos(grid: number, win: ExtContainer, startrow: number, startcol: number, width: number, height: number): void,
  win_float_pos(grid: number, win: ExtContainer, anchor: string, anchor_grid: number, anchor_row: Float, anchor_col: Float, focusable: boolean): void,
  win_external_pos(grid: number, win: ExtContainer): void,
  win_hide(grid: number): void,
  win_close(grid: number): void,
  msg_set_pos(grid: number, row: number, scrolled: boolean, sep_char: string): void,
  win_viewport(grid: number, win: ExtContainer, topline: number, botline: number, curline: number, curcol: number): void,
  popupmenu_show(items: any[], selected: number, row: number, col: number, grid: number): void,
  popupmenu_hide(): void,
...
export interface Api {
  ...
  inputMouse(button: string, action: string, modifier: string, grid: number, row: number, col: number): void,
  ...
  selectPopupmenuItem(item: number, insert: boolean, finish: boolean, opts: object): void,
...
export interface Buffer {
  ...
  setKeymap(buffer: Buffer, mode: string, lhs: string, rhs: string, opts: object): void,
  ...
  getExtmarkById(buffer: Buffer, ns_id: number, id: number, opts: object): Promise<number[]>,
  getExtmarks(buffer: Buffer, ns_id: number, start: any, end: any, opts: object): Promise<any[]>,
  setExtmark(buffer: Buffer, ns_id: number, line: number, col: number, opts: object): Promise<number>,
  ...
  clearNamespace(buffer: Buffer, ns_id: number, line_start: number, line_end: number): void,
  setVirtualText(buffer: Buffer, src_id: number, line: number, chunks: any[], opts: object): Promise<number>,
  call(buffer: Buffer, fun: LuaRef): Promise<any>,
  getNumber(buffer: Buffer): Promise<number>,
  clearHighlight(buffer: Buffer, ns_id: number, line_start: number, line_end: number): void,
}
